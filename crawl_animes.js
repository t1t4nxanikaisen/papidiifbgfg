...
// Simple CLI: node crawl_animes.js [startUrl] [outFile] [maxPages] [delayMs]
const args = process.argv.slice(2);
const START_URL = args[0] || 'https://toonstream.love/category/cartoon/';
const OUT_FILE = args[1] || path.join(__dirname, 'data', 'animes.json');
const MAX_PAGES = parseInt(args[2], 10) || 500;
...
async function crawl(startUrl, outFile, maxPages = 500, delayMs = 500) {
    ...
    while (url && page <= maxPages) {
        ...
        try {
            ...
            if (consecutiveEmpty >= 5) {
                console.log('No new items for 5 consecutive pages — stopping.');
                break;
            }
    ...
    try {
        ...
        console.log(`Saved ${results.length} animes to ${outFile}`);
    } catch (e) {
